	...
	$(".comment-btn").click(function(){
		...
		$.post("comment/add",{
			...
			if(data.result=="success"){
				humane.log("评论成功");
				window.location="user/oindex?id="+userid;
			}else{
...
function followFan(id){
	$.post("follow/add",{
		...
		if(data.result=="success"){
			humane.log("关注成功");
			window.location="user/oindex?id="+id;
		}else{
			humane.log(data.result); 
		}
	},"json");
...
function getOpenClassGroupByPage(currentPageIndex){
	  var itemsPerPage = 10;
	  var id=$("#hide-o-id").val();
	  window.location = "user/oindex?id="+id+"&pageIndex="+currentPageIndex+"&itemsPerPage="+itemsPerPage;
}
function addGroup(){
